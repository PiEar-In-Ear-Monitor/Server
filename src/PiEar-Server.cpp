...
auto main(int argc, char *argv[]) -> int {
    try {
        ...
        std::atomic<int> cpm = 100, audio_index;
        ...
        PiEar::task task = PiEar::task(&channels, filename, 5, &audio_index);
        ...
        std::thread webserver_thread(PiEar::mainloop_http_server, &kill_http, &channels, &cpm, PiEar::random_string(20),
                                     1, info, &audio_index, &change_audio_source);
        ...
        while (!PiEar::SIG_HANDLER_KILL_SERVER) {
            if (change_audio_source) {
                ...
                PIEAR_LOG_WITHOUT_FILE_LOCATION(boost::log::trivial::info) << "Changing audio source to "
                                                                           << audio_index;
                ...
                if (audio.channel_count(audio.get_audio_index()) < 1) {
                    PIEAR_LOG_WITH_FILE_LOCATION(boost::log::trivial::error)
                        << "No audio channels found on new source: " << audio_index;
                    kill_click = true;
                    click_thread.join();
                    Pa_Terminate();
                ...
                webserver_thread = std::thread(PiEar::mainloop_http_server, &kill_http, &channels, &cpm,
                                               PiEar::random_string(20), 1, info, &audio_index, &change_audio_source);
        ...
        kill_http = kill_multicast = kill_audio = kill_click = true;
        task.async_stop_save_task();
        audio_thread.join();
        click_thread.join();
        webserver_thread.join();
